...
function Console:Create(settings)
	...
	box.FocusLost:connect(function(_)
		if ConsoleFrame.Visible then
            ...
            local first = tostring(inputsep[1])
			
			if _ then
				newOutput(input)
			end;

            if rawget(Console.commands, first) then
                rawget(Console.commands, first)(table.concat(inputsep, ' ', 2))
            end;
...
function esp.new(settings)
	assert(#settings==0,'provide settings pls')
    ...
    if type(folder) == 'table' then
		local largerFolder = Instance.new('Folder',esp.gui)
		largerFolder.Name = 'Larger_Folder'
        for i,v in pairs(folder) do
            esp.new({folder=largerFolder})
        end
        return
    end
	
	-- assert(#folder:GetChildren()~=0,'empty folder...')
	
	local F = Instance.new('Folder',esp.gui)
	F.Name = folder.Name
	...
	local function makeESP(p,c,t)
		if p:IsA('BasePart') then
			...
			p:GetPropertyChangedSignal('Parent'):Connect(function(parent)
				if parent==nil and esp.current[folder] and esp.current[folder][curp] then
                    -- print(esp.current[folder][curp])
					esp.current[folder][curp]:Destroy()
					esp.current[folder][curp]=nil
				end
			end)
		end
	end
